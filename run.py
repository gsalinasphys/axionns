...
length, nparticles, batch_size, conservation_check, rprecision = np.array([-1,1])/300, int(1500), int(1e2), False, 2e-3
...
for i, hit in enumerate(chosen_hits):
    conversion.append(np.append(hit, probabilities[i]))

np.save(output_dir + event + '/' + event + '_conversion.npy', conversion)
...
if draw_plots:
    ...
    for single_particle in single_particles[:1000]:
        ax.scatter(single_particle.T[3], single_particle.T[2], s = 1)
...
############################ Histogram of conversion probabilities ###########################################
