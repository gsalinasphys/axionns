...
def mkdir_event(NS, clump, r_in, v_in, impact_param, r_roche, v_roche, bmax, size=6, chars=string.ascii_uppercase):     # Variable 'clump' should be an instance of AS or MC
    ...
    f.write('Impact parameter: ' + '{:.2e}'.format(impact_param) + ' km\n\n' )

    f.write('-'*25 + ' Trajectories ' + '-'*50 + '\n')
    f.write('Position at Roche radius: ' + '[ ' + '{:.2e}'.format(np.array(r_roche[0])) + ' ' + '{:.2e}'.format(np.array(r_roche[1])) + ' ' + '{:.2e}'.format(np.array(r_roche[2])) + ' ] km\n')
    f.write('Velocity at Roche radius: ' + '[ ' + '{:.2e}'.format(np.array(v_roche[0])) + ' ' + '{:.2e}'.format(np.array(v_roche[1])) + ' ' + '{:.2e}'.format(np.array(v_roche[2])) + ' ] km/s\n' )
    if clump.clump_type == 'Axion Minicluster (NFW profile)':
        f.write('Sampling cylinder radius: ' + '{:.2e}'.format(np.array(bmax)) + ' km\n')
